...
def convert_vector_to_bearing_inclination(vector):
    ...
    bearing = int(
        round(
            np.degrees(
                math.acos(
                    np.dot([-1, 0, 0], [x, y, 0]) / (1 * np.sqrt(x**2 + y**2))
                )
            ),
            0,
        )
    ...
    incl = int(
        round(
            np.degrees(
                math.acos(
                    np.dot([x, y, 0], [x, y, z])
                    / (np.sqrt(x**2 + y**2 + z**2) * np.sqrt(x**2 + y**2))
                )
            ),
            0,
        )
...
def fs(nn: int, deg: list):
    ...
    return np.array(
        list(map(lambda ldeg, lnn: math.sin(math.radians(ldeg)) ** lnn, deg, nnn))
    )
...
def fc(nn: int, deg: list):
    nnn = np.repeat(nn, len(deg))
    return np.array(
        list(map(lambda ldeg, lnn: math.cos(math.radians(ldeg)) ** lnn, deg, nnn))
    )
...
def calMF(x0, data):
    ...
    depth = dep - tdepth
    alphaVV = den
    alphaNE = 0.5 * (alphaNN - alphaEE) * 2.0 * SNE0 / (SN0 - SE0)
    alphaEV = 0.5 * (alphaEE - alphaVV) * 2.0 * SEV0 / (SE0 - SV0)
    alphaVN = 0.5 * (alphaVV - alphaNN) * 2.0 * SVN0 / (SV0 - SN0)
    ...
    # Synn
    psc_final = (
        SN * fc(2.0, pi) * fc(2.0, psi)
        + SE * fc(2.0, pi) * fs(2.0, psi)
        + SV * fs(2.0, pi)
        + SNE * fc(2.0, pi) * fs(1.0, 2.0 * psi)
        + SEV * fs(1.0, 2.0 * pi) * fs(1.0, psi)
        + SVN * fs(1.0, 2.0 * pi) * fc(1.0, psi)
    )
    # Mizuta  - non-correct
    ...
    errsum = (psm - psc_final) ** 2.0 + (fSV - (over + den * depth)) ** 2.0
    errsum = (errsum.sum() / (len(errsum) - 1)) ** 0.5

    return errsum


@st.cache_data()
def BH_and_wsm_func(WSM_file, lat, lng, near_pts):
    """
    wsm 파일명, 위도, 경도 정보를 수신하여 wsm 파일에서
    해당 위도, 경도에 가장 가까운 측정소 (near_pts)개를 찾아
    가까운 순서대로 near_pts 행만큼 먼저 반환하고, 나머지를 반환함
    """

    wsm = pd.read_csv(WSM_file, encoding="ISO-8859-1", engine="python")
    kor = wsm[wsm["COUNTRY"] == "Korea - Republic of"]

    kor["dist_wsm_src(km)"] = list(
        map(
            lambda slat, slng: round(haversine((lat, lng), (slat, slng), unit="km"), 2),
            kor["LAT"],
            kor["LON"],
        )
    )
    ...
    return kor_sorted[0:near_pts], kor_sorted[near_pts::]
...
if __name__ == "__main__":
    ...
    st.sidebar.markdown(
        f"지하공간 형태나 균열대 분포에 따라 경사시추공을 이용한 삼차원 초기지압 측정이 필요할 때 본 코드를 이용하여 현지 측정값과 계산값 간의 오차를 최소화함으로써 응력을 추정함."
    )
    st.sidebar.markdown("**참고자료**")
    st.sidebar.markdown(
        f"[이론적 배경 (Synn et al., 2015)](http://dx.doi.org/10.1016/j.ijrmms.2015.01.012)"
    )
    st.sidebar.markdown(
        f"[최적화 함수(BFGS)](https://docs.scipy.org/doc/scipy/reference/generated/scipy.optimize.minimize.html#scipy.optimize.minimize)"
    )
    st.sidebar.markdown(
        f"[World Stress Map 안내서 및 데이터 파일](https://datapub.gfz-potsdam.de/download/10.5880.WSM.2016.001/)"
    )
    st.sidebar.markdown(f"---")

    # dat file upload
    dfile = st.sidebar.file_uploader(
        label="**데이터 파일 선택**", type=["dat", "txt"], on_change=ini_reload()
    )
    ...
    if dfile is None:
            ...
            try:
                ...
                with tab1:
                    with st.spinner(f"평면도 생성 중"):
                        ...
                        # folium.WmsTileLayer('https://data.kigam.re.kr/openapi/wms', layers, styles='', \
                        # FORMAT='image/png', transparent=True, version='1.1.1', attr='', name="5만 지질도", overlay=True, control=True, show=False, key=api_key).add_to(m)

                        folium.Marker([lat, lng], popup=test, tooltip=test).add_to(m)

                        #
                        wsm_near_five, wsm_others = BH_and_wsm_func(
                            WSM_file, lat, lng, near_pts
                        )
                        ...
                        st.markdown(f"측정지점과 가장 가까운 {near_pts}개의 WSM 데이터")
                        AgGrid(
                            wsm_near_five,
        ...
        result = minimize(calMF, x0, data, tol=1.0e-5, method="BFGS")

        fi, mdep, cz0, psm, cburden, cden = (
            data[:, 0],
            data[:, 6],
            data[:, 2],
            data[:, 7],
        ...
        for idx, [PN, PE, PV, PNE, PEV, PVN] in df[
            ["PN", "PE", "PV", "PNE", "PEV", "PVN"]
        ].iterrows():
            sigma = np.asarray([[PN, PNE, PVN], [PNE, PE, PEV], [PVN, PEV, PV]])
        ...
        t1 = pd.DataFrame(
            [convert_vector_to_bearing_inclination(xx) for xx in df["P1vec"]]
        )
        df["P1_bear."] = t1.iloc[:][0]
        df["P1_incl."] = t1.iloc[:][1]
        t2 = pd.DataFrame(
            [convert_vector_to_bearing_inclination(xx) for xx in df["P2vec"]]
        )
        df["P2_bear."] = t2.iloc[:][0]
        df["P2_incl."] = t2.iloc[:][1]
        t3 = pd.DataFrame(
            [convert_vector_to_bearing_inclination(xx) for xx in df["P3vec"]]
        )
        ...
        anno = (
            anno
            + f"PE={1./PNEV_results[1].params[1]:0.3f}*Depth{anno_func(PNEV_results[1].params[0]/PNEV_results[1].params[1])} (r2={PNEV_results[1].rsquared:0.2f})<br>"
        )
        anno = (
            anno
            + f"PV={1./PNEV_results[2].params[1]:0.3f}*Depth{anno_func(PNEV_results[2].params[0]/PNEV_results[2].params[1])} (r2={PNEV_results[2].rsquared:0.2f})<br>"
        )
        ...
        fig4 = px.scatter(
            ...
            trendline="ols",
            labels=dict(
                X="PNE or PEV or PVN (MPa)", mdepth="Depth (m)", Legend="Stress"
            ),
            height=800,
            width=1000,
        )
        ...
        anno = (
            anno
            + f"PEV={1./PNEV_results[1].params[1]:0.3f}*Depth{anno_func(PNEV_results[1].params[0]/PNEV_results[1].params[1])} (r2={PNEV_results[1].rsquared:0.2f})<br>"
        )
        anno = (
            anno
            + f"PVN={1./PNEV_results[2].params[1]:0.3f}*Depth{anno_func(PNEV_results[2].params[0]/PNEV_results[2].params[1])} (r2={PNEV_results[2].rsquared:0.2f})<br>"
        )
        ...
        fig6, ax = plt.subplots(
            2, 2, subplot_kw=dict(projection="3d"), figsize=(10, 10)
        )

        plt.rcParams["grid.linewidth"] = 0.3
        plt.tight_layout()

        views = [[(20, 15, 0), (90, 0, 0)], [(0, 0, 0), (0, 90, 0)]]
        ...
        for i in range(2):
            for j in range(2):
                ...
                ax[i][j].quiver(
                    0, 0, 0, -1, 0, 0, color="k", label=None, arrow_length_ratio=0.1
                )
                ax[i][j].quiver(
                    0, 0, 0, 0, 1, 0, color="k", label=None, arrow_length_ratio=0.1
                )
                ax[i][j].quiver(
                    0, 0, 0, 0, 0, -1, color="k", label=None, arrow_length_ratio=0.1
                )
                ...
                for idx, [[x1, y1, z1], [x2, y2, z2], [x3, y3, z3]] in df[
                    ["P1vec", "P2vec", "P3vec"]
                ].iterrows():
                    ax[i][j].quiver(
                        0,
        ...
        ## Make a density contour plot of the orientation
        ...
        fig7 = px.scatter(
            ...
            trendline="ols",
            labels=dict(
                X="Principal stress (MPa)",
                mdepth="Depth (m)",
                Legend="Principal stress",
            ),
            height=800,
            width=1000,
        )
        ...
        anno = (
            anno
            + f"P2={1./PRINCIPAL_results[1].params[1]:0.3f}*Depth{anno_func(PRINCIPAL_results[1].params[0]/PRINCIPAL_results[1].params[1])} (r2={PRINCIPAL_results[1].rsquared:0.2f})<br>"
        )
        anno = (
            anno
            + f"P3={1./PRINCIPAL_results[2].params[1]:0.3f}*Depth{anno_func(PRINCIPAL_results[2].params[0]/PRINCIPAL_results[2].params[1])} (r2={PRINCIPAL_results[2].rsquared:0.2f})<br>"
        )
        ...
        fig8 = px.scatter(
            ...
            trendline="ols",
            labels=dict(
                X="Principal stress ratio", mdepth="Depth (m)", Legend="Legend"
            ),
            height=800,
            width=1000,
        )
        ...
        anno = (
            anno
            + f"P2/P3={1./PR_results[1].params[1]:0.3f}*Depth{anno_func(PR_results[1].params[0]/PR_results[1].params[1])} (r2={PR_results[1].rsquared:0.2f})<br>"
        )
        anno = (
            anno
            + f"P1/P3={1./PR_results[2].params[1]:0.3f}*Depth{anno_func(PR_results[2].params[0]/PR_results[2].params[1])} (r2={PR_results[2].rsquared:0.2f})<br>"
        )
        ...
        with tab2:
            ...
            st.markdown(
                f"Total depth from ground surface to upper boundary of bedrock (m): {tdepth}"
            )
        ...
        with tab6:
            ...
            with col1:
                AgGrid(
                    ...
                    columns_auto_size_mode=ColumnsAutoSizeMode.FIT_CONTENTS,
                    update_on=["init", "gridChanged", "viewportChanged"],
                )
            with col2:
